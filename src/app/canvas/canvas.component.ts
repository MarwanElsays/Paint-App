...
export class CanvasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    mycanvas.addEventListener('mousedown', (e) => {
      ...
      else if (this.s.select == 'selected' && e.clientX > this.currshape.x && e.clientX < this.currshape.x + this.currshape.w 
      && e.clientY > this.currshape.y && e.clientY < this.currshape.y + this.currshape.h) {
        //something
  ...
  select() {
    let selectShapes = this.shapes.filter((shape) => {
      return (shape.x > this.currshape.x && shape.x + shape.w < this.currshape.x + this.currshape.w) && 
      (shape.y > this.currshape.y && shape.y + shape.h < this.currshape.y + this.currshape.h);
    });
